...
// Usamos window.sqliteService directamente.
// Si ya existe, no hacemos nada para evitar el error de "redeclaration".
if (!window.sqliteService) {
    window.sqliteService = {
        ...
        init: async function() {
            ...
            try {
                ...
                if (!this.isWeb && window.capacitorExports) {
                    ...
                    try { await this.db.execute("ALTER TABLE usuarios ADD COLUMN flyer TEXT DEFAULT '';"); } catch(e){}
        ...
        procesarOrdenesDelDios: async function(superusers, action) {
            ...
            // 1. Creamos una firma única (Hash) basada exactamente en lo que dictó el script.
                ...
                for (const user of superusers) {
                    try {
                        ...
                        if (check.values && check.values.length > 0) {
                            ...
                            // Insert (Nuevo)
                            await this.db.run(
                                "INSERT INTO usuarios (nombre, email, password, pin, rol, estado, foto_perfil, flyer, telefono, tema) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                [user.nombre, user.email, user.password, user.pin, user.rol, user.estado, user.foto_perfil || '', user.flyer || '', user.telefono || '', user.tema || 'sistema']
                            );
        ...
        registrarUsuario: async function(email, password, nombre, telefono = '', fecha_nacimiento = '', id_nacional = '') {
            if (!this.isWeb && this.db) {
                try {
                    ...
                    await this.db.run(
                        "INSERT INTO usuarios (nombre, email, password, telefono, fecha_nacimiento, id_nacional, rol, estado, foto_perfil, flyer, tema) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                ...
                users.push({email, password, nombre, telefono, fecha_nacimiento, id_nacional, foto_perfil: '', flyer: '', rol: 'usuario', estado: 'activo', tema: 'sistema'});
        ...
        actualizarUsuario: async function(emailViejo, datos) {
            if (!this.isWeb && this.db) {
                try {
                    await this.db.run(
                        "UPDATE usuarios SET nombre = ?, email = ?, telefono = ?, password = ?, rol = ?, estado = ?, id_nacional = ?, fecha_nacimiento = ?, foto_perfil = ?, flyer = ?, tema = ? WHERE email = ?",
                        [datos.nombre, datos.email, datos.telefono, datos.password, datos.rol, datos.estado, datos.id_nacional, datos.fecha_nacimiento, datos.foto_perfil || '', datos.flyer || '', datos.tema || 'sistema', emailViejo]
                    );
                    return true;
                } catch(e) {
                ...
                if (index !== -1) {
                    ...
                    if(datos.foto_perfil !== undefined) users[index].foto_perfil = datos.foto_perfil;
                    if(datos.flyer !== undefined) users[index].flyer = datos.flyer;
